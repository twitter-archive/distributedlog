...
public class DistributedLogConfiguration extends CompositeConfiguration {
    ...
    public static final int BKDL_READLACLONGPOLL_TIMEOUT_DEFAULT = 10000;
    ...
    public static final String BKDL_ENABLE_SIBLING_LOCK_FOR_WRITE_LOCK = "enableSiblingLockForWriteLock";
    public static final boolean BKDL_ENABLE_SIBLING_LOCK_FOR_WRITE_LOCK_DEFAULT = false;
    ...
    // White-listed stream-level configuration settings.
    ...
    /**
     * Whether to enable sibling lock for write lock. Enabling sibling lock means when a lock waiter
     * finds the lock owner is from same zookeeper session and it is the sibling znode in lock waiters list,
     * it would claim ownership. Disable sibling lock, it would reject to claim ownership.
     *
     * Enabling sibling lock will avoid self-deadlock when forgetting to release lock gracefully.
     *
     * @return flag to enable sibling lock for write lock
     */
    public boolean getEnableSiblingLockForWriteLock() {
        return this.getBoolean(BKDL_ENABLE_SIBLING_LOCK_FOR_WRITE_LOCK,
                               BKDL_ENABLE_SIBLING_LOCK_FOR_WRITE_LOCK_DEFAULT);
    }

    /**
     * Set the flag to enable/disable sibling lock for write lock.
     *
     * @param enabled
     *          flag to enable/disable sibling lock for write lock.
     * @return distributed log configuration.
     */
    public DistributedLogConfiguration setEnableSiblingLockForWriteLock(boolean enabled) {
        setProperty(BKDL_ENABLE_SIBLING_LOCK_FOR_WRITE_LOCK, enabled);
        return this;
    }
